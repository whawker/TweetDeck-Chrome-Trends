            ...
            if(column.find('div.scroll-content').is(':empty')){
                $.getJSON('https://api.twitter.com/1/trends/' +a.getTrendLocationWoeid() +'.json',
                    ...
                    function(data) {
                        ...
                        $.each(data[0].trends, function(i,item){
                            var t = $(trendItem);
                            t.find('header').append('<a href="' +item.url +'" onclick="TD.ui.openColumn.showSearch(\'' +(item.name).replace('\'', '\\\'') +'\');"><strong class="fullname">'+item.name +'</strong></a>');
                            column.find('div.scroll-content').append(t);
                        });
...
TD.services.TwitterClient.prototype.makeTwitterCall = function (a, b, c, d, e, f, g) {
    ...
    var h=this,i=function(a){if(e)try{a=e.call(h,a)}catch(b){console.log("Error processing Twitter data",a,b),g(b);return}f&&f(a)};b&&(b.include_entities=1,b.include_user_entities=1,b.include_cards=1),c=="GET"?this.get(a,b,d,i,g):this.post(a,b,null,null,i,g)
}